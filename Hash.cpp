...
int midSquareHash(int key, int tableSize) {
    int squared = key * key;
    int mid = (squared / 100) % 100; // Extracting the middle two digits
...
public:
    ...
    void insert(int key) {
        int index = divisionHash(key, tableSize);
        while (table[index] != -1 && table[index] != DELETED) { // Linear probing
    ...
    bool search(int key) {
        ...
        while (table[index] != -1) {
            ...
            index = (index + 1) % tableSize;
            if (index == startIndex) break; // Loop detected
        }
        return false;
    }
...
public:
    ...
    void insert(int key) {
        ...
        while (table[index] != -1 && table[index] != DELETED) {
            ...
            if (i >= tableSize) {  // If we've tried all possible positions
    ...
    bool search(int key) {
        ...
        do {
            if (table[index] == key) return true;
            if (table[index] == -1) return false;  // Empty slot means key not found
